...
function Invoke-mlMSSqlDataReader {
<#
.SYNOPSIS
    Runs a select statement query against a SQL Server database.
.DESCRIPTION
    Invoke-mlMSSqlDataReader is a PowerShell function that is designed to query
...
.EXAMPLE
     Invoke-mlMSSqlDataReader -ServerInstance Server01 -Database Master -Query '
     select name, database_id, compatibility_level, recovery_model_desc from sys.databases'
.EXAMPLE
     'select name, database_id, compatibility_level, recovery_model_desc from sys.databases' |
     Invoke-mlMSSqlDataReader -ServerInstance Server01 -Database Master
.EXAMPLE
     'select name, database_id, compatibility_level, recovery_model_desc from sys.databases' |
     Invoke-mlMSSqlDataReader -ServerInstance Server01 -Database Master -Credential (Get-Credential)
.INPUTS
    String
.OUTPUTS
    DataRow
...
#>
    ...
    param (
        ...
        [Parameter(Mandatory,ValueFromPipeline)][string]$Query,
    ...
    BEGIN {
        ...
        catch { Write-Error -Message "An error has occurred. Error details: $($_.Exception.Message)" }
    ...
    PROCESS {
        ...
        catch { Write-Error -Message "An error has occured. Error Details: $($_.Exception.Message)" }
