...
#>
function Show-mlNetworkInterfaces {
    ...
    $objNICs = [System.Net.NetworkInformation.NetworkInterface]::GetAllNetworkInterfaces()
    ...
    # First, check if interfaces are found
     if (!$objNICs -OR $objNICs.Length -LT 1) {
            "  No network interfaces found."
            RETURN
        }

    # Here print out number of interfaces and interface details
    [System.Console]::WriteLine("  Number of interfaces .................... : {0}" -f $objNICs.Length)
    Foreach ($itmAdapter in $objNICs) {
        $objProperties = $itmAdapter.GetIPProperties();
        " ";""
        "{0}" -F $itmAdapter.Description
        "=" * $itmAdapter.Description.Length
        "  Interface type .......................... : {0}" -F $itmAdapter.NetworkInterfaceType
        [System.Console]::Write("  Physical address ........................ : ")
        $address = $itmAdapter.GetPhysicalAddress();
        ...
        for($i = 0; $i -lt $bytes.Length; $i++) {
            ...
            if ($i -NE $bytes.Length -1) { [System.Console]::Write("-") }
        }
